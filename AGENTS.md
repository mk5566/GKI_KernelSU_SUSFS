...
- Keep full GKI hardware compatibility without artificial symbol trimming: export all kernel symbols (`TRIM_NONLISTED_KMI=0`, `KMI_ENFORCED=0`) and allow OEM vendor module loading (`CONFIG_MODULE_SIG_FORCE=n`) so device proprietary drivers (display, touch, GPU, PMIC) load properly. Never change `kernel/module.c` to accept symbol-version mismatches.
- Package Header v4 `boot.img` with a signed RSA-2048 AVB hash footer at the end of ishtar's measured 192 MiB `boot_a`/`boot_b` partition. The old 64 MiB footer was incorrect. The generated test key is not the key in the currently booted image; packaging success alone does not qualify the image for flashing.
- Preserve the phone's built-in zRAM/zsmalloc and usable compressor path. Google's unmodified GKI defconfig has these as modules, while the captured system_dlkm copies have 5.15.194 vermagic; the built-in LZ4KD candidate must still pass canonical defconfig and module-list validation before it is qualified.
- Pass Python tests, strict repo guard, full Linux build, and packaging review before proposing any flashable image. A Windows dry run or raw `Image` is not a device-qualified release.
