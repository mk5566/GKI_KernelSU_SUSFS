...
## Current patch-safety disposition — 2026-09-23

[PATCH_SAFETY_AUDIT.md](PATCH_SAFETY_AUDIT.md) supersedes the older selectable-series
...
no device-runtime cost and do not establish equal benchmark performance.

Repository validation now passes 87 tests, including synthetic Git mutation
and failure-path tests. These are not kernel execution tests. No full kernel
build, canonical defconfig run, ABI/KMI diff, module load, or phone test was
performed in this audit. The uploaded project lacks a complete common tree.

## Historical 2026-09-23 follow-up (superseded patch selection)

The following records the earlier work and its then-current decisions. Claims
of previous exact kernel patch application are inherited audit records, not
newly reproduced results. In particular, the statements that ten patches are
selectable and that BBRv3 is available no longer describe the delivered tree.
...
## Historical initial change requests (consult current audit for status)
...
| Workflow choices can create unqualified build variants. | Keep GitHub Actions input-free and pass the audited fixed profile directly to the builder. | Deterministic CI output and smaller validation surface. | None on device. | Workflow contract tests verify manual-only, input-free dispatch and fixed flags. |
...
- Binder and GPU tuning remain deferred. The retired suspend, IRQ and BBRv3 overrides cannot be selected; any replacement needs a separately reviewed implementation and qualification, not a workflow toggle.
